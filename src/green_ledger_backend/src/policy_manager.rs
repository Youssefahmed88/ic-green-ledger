...
use ic_cdk::{update, query}; // Added query for get_policy_details, get_historical_subscribers
...
pub fn reset_season_state(policy_id: u64) -> Result<()> {
    ...
    policy.last_subscribed_season.insert(caller, policy.season); // This line seems to incorrectly update last_subscribed_season for the caller to the *old* season. If it's meant to track the last season they subscribed to *this* policy, it should be `policy.season + 1`. For now, keeping as is to match original logic, but noting potential logical issue.
    policy.season += 1;
    policy.status = PolicyStatus::Active;
